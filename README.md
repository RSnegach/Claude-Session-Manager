...
- **Real-time status indicators**: shows each session's state: `READY`, `THINKING`, `APPROVE?`, `DECISION`, `REJECTED`, or `INTERRUPTED`. `READY` appears only when the last turn actually ended (`end_turn`); the manager never infers "done" from a quiet log, so a session mid-thinking or running a long tool call stays `THINKING` and won't flicker to `READY`. `DECISION` (purple) flags a session blocked on a question that needs a human answer (`AskUserQuestion` or plan approval) — these are *not* auto-answered by bypass mode, so they stand apart from routine `APPROVE?` permission prompts. Click an `APPROVE?` or `DECISION` flag to jump straight to that session's window
- **Auto-approve via permission mode**: the green **A** button sets a session's `permissions.defaultMode` to `bypassPermissions` (runs every tool call without prompting) when on, and back to `default` when off. Claude Code reloads this live, so prompts are never missed, duplicated, or accidentally top-selected on dropdowns. As a safety net, if a permission prompt slips through on an auto-approve session and `APPROVE?` lingers past ~3s, the manager sends a single `Enter` to clear it (this never fires on a `DECISION`, so it can't auto-pick a question/plan menu). The header toggle flips every session at once; each row's **A** overrides per session. Settings writes are merge-only (your allow-lists are preserved) and the original `defaultMode` is restored on toggle-off, session close, and app exit. Note: permission mode is scoped to the working directory, so sessions launched from the **same folder** share one auto-approve state
...
| **APPROVE? / DECISION flag** | Click the flag to focus that session's window so you can answer the pending prompt |
...
4. **Auto-approve**: reads each session's working directory from its JSONL `cwd` field and writes `permissions.defaultMode` into that project's `.claude/settings.local.json` (`bypassPermissions` when on, `default` when off), which Claude Code reloads live. Writes are atomic and merge-only; the original `defaultMode` is captured and restored on toggle-off, session close, and exit (persisted so a crash can't strand a file in bypass). If an `APPROVE?` prompt slips through on an auto-approve session and persists past ~3s, the manager sends a single `Enter` (via `WriteConsoleInputW`, `PostMessage WM_KEYDOWN` fallback) to that session's console only — gated to `APPROVE?` so it never touches a `DECISION` menu
